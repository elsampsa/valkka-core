...
TestFileStream::TestFileStream(const char *filename)
{
    ...
    { // this ****er reserves and AVPacket and sometimes does not free it properly => memleak .. actually, this happens only, when using av_read_frame afterwards in pull().
...
FileStream::FileStream(FileContext &ctx) : ctx(ctx)
{
  ...
  */
    ...
    { // send setup frames describing the file contents
        AVCodecID codec_id = input_context->streams[i]->codec->codec_id;
        ...
        { // TODO: check if this is in AVCC or Annex B (how?)
...
void FileStream::seek(long int ms_streamtime_)
{
  ...
  */
    ...
    i = avformat_seek_file(input_context, -1, std::min((long int)0, ms_streamtime_*1000 - 5000), ms_streamtime_*1000, ms_streamtime_*1000 + 5000, 0);
...
long int FileStream::update(long int mstimestamp)
{ // update the target time .. (only when playing)
    long int timeout;
    if (state == FileState::stop)
    {
        return Timeout::filethread;
    }
    if (state == FileState::play or state == FileState::seek)
    { // when playing, target time changes ..
...
long int FileStream::pullNextFrame()
{
    ...
    if (dt > 0)
    { // so, this has been called in vain.. must wait still
#ifdef FILE_VERBOSE
    ...
    else
    {
        ...
        ///*
        if (!filters[index])
        { // no filters for this codec
            out_frame.copyFromAVPacket(avpkt);
        }
        else
        { // there's a filter ..
            if ((codec_id == AV_CODEC_ID_H264) and (avpkt->size >= 4) and (data[0] == 0 and data[1] == 0 and data[2] == 0))
            { // must be annex b .. no need to use the filter
                out_frame.copyFromAVPacket(avpkt);
            }
            else
            { // use the filter
    ...
    */
        ...
        if (((long int)avpkt->pts) <= frame_mstimestamp_)
        { // same timestamp!  recurse (typical for sequences: sps, pps, keyframe)
#ifdef FILE_VERBOSE
...
void FileThread::handleSignals()
{
    ...
    for (std::deque<FileSignalContext>::iterator it = signal_fifo.begin(); it != signal_fifo.end(); ++it)
    { // it == pointer to the actual object (struct SignalContext)

        switch (it->signal)
        {
        case FileSignal::exit:
            for (i = 0; i <= I_MAX_SLOTS; i++)
            { // stop and deregister all streams
                file_context.slot = i;
...
int FileThread::safeGetSlot(SlotNumber slot, FileStream *&con)
...
void FileThread::openFileStream(FileContext &file_ctx)
{
    ...
    case 0: // slot is free
        ...
        else
        {
            ...
            file_ctx.duration = file_stream->duration/1000; // outgoing signal // from microsecs to millisecs
        }
        break;
