...
bool ForkFrameFilterN::connect(const char *tag, FrameFilter *filter)
{
    ...
    // std::cout << "CONNECTED " << nametag << std::endl;
...
void ForkFrameFilterN::go(Frame *frame)
...
void DumpFrameFilter::go(Frame *frame)
{ // du -h *.bin | grep "s0"
...
void CountFrameFilter::go(Frame *frame)
{ // du -h *.bin | grep "s0"
    count++;
...
void TimestampFrameFilter2::go(Frame *frame)
{
    ...
    if ((frame->mstimestamp - savedtimestamp) > DEFAULT_TIMESTAMP_RESET_TIME)
    { // reset the correction once in a minute
        mstime_delta = 0;
...
void RepeatH264ParsFrameFilter::run(Frame *frame)
{
    ...
    else
    {
        ...
        if (basic_frame->codec_id == AV_CODEC_ID_H264)
        {
            ...
            else if (slice_type == H264SliceType::i)
            { // KEY
                if (phase == 1)
...
void GateFrameFilter::run(Frame *frame)
{
    ...
    if (on)
    { // pass all frames if flag is set
        (this->next)->run(frame);
    }
    else if (frame->getFrameClass() == FrameClass::setup and config_frames)
    { // .. if flag is not set, pass still config frames if config_frames is set
        (this->next)->run(frame);
    }
}
...
void CachingGateFrameFilter::run(Frame *frame)
{
    ...
    if (on)
    { // passes all frames if flag is set
        (this->next)->run(frame);
    }
    if (frame->getFrameClass() == FrameClass::setup)
    {
        setupframe = *(static_cast<SetupFrame *>(frame)); // create a cached copy of the SetupFrame
        if (setupframe.sub_type == SetupFrameType::stream_init)
        { // TODO: shouldn't we have an array of setupframes (for each substream)
            got_setup = true;
        }
    }
}
...
void TimeIntervalFrameFilter::run(Frame *frame)
{
    ...
    std::cout << std::endl
              << "TimeIntervalFrameFilter: " << std::endl;
#endif
    if (!this->next)
    {
        return;
    } // calls next filter .. if there is any
#ifdef INTERVAL_VERBOSE
    std::cout << std::endl
              << "TimeIntervalFrameFilter: mstimestamps=" << frame->mstimestamp << " " << prevmstimestamp << std::endl;
    std::cout << std::endl
              << "TimeIntervalFrameFilter: delta       =" << (frame->mstimestamp - prevmstimestamp) << std::endl;
#endif
    if ((frame->mstimestamp - prevmstimestamp) >= mstimedelta)
    {
#ifdef INTERVAL_VERBOSE
        std::cout << std::endl
                  << "TimeIntervalFrameFilter: WRITE" << std::endl;
#endif
...
void SwScaleFrameFilter::run(Frame *frame)
{ // AVThread calls this ..
...
void SwScaleFrameFilter::go(Frame *frame)
{ // do the scaling
    if (!this->next)
    {
        return;
    } // exit if no next filter
