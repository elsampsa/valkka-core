...
bool FrameFifo::writeCopy(Frame *f, bool wait)
{
    ...
    if (fifo.size() > 1)
    {
        std::cout << "FrameFifo: " << name << " writeCopy: count=" << fifo.size() << std::endl;
    }
#endif
...
Frame *FrameFifo::read(unsigned short int mstimeout)
{
    ...
    if (fifo.size() > 1)
    {
        std::cout << "FrameFifo: " << name << " read: count=" << fifo.size() << std::endl;
    }
#endif
...
bool FDFrameFifo::writeCopy(Frame *f, bool wait)
{
    ...
    if (fifo.size() > 1)
    {
        std::cout << "FDFrameFifo: " << name << " writeCopy: count=" << fifo.size() << std::endl;
    }
#endif
